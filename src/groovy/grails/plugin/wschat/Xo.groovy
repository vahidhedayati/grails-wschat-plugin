...
@ServerEndpoint("/xo/{gameId}/{username}")
public class Xo  implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent servletContextEvent) {
		...
		try {
			serverContainer?.addEndpoint(Xo.class)
			// Keep chat sessions open for ever
			def config=Holders.config
			int DefaultMaxSessionIdleTimeout=config.xo.timeout  ?: 0
