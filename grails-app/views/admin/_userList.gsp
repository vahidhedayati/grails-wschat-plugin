...
<table class="col-sm-12">
				<thead>
					<tr>
					<th>
						<a onclick="<g:remoteFunction controller="wsChat"  action="viewUsers" update="${divupdate }"  
...
<g:if test="${uList.contains(users?.username) }">
...
<li><a  data-toggle="modal" href="#banuser1"  onclick="closeAdminModal();javascript:banuser('${users?.username}');">Ban  ${users?.username}</a></li>
	...
	<div 
	<g:if test="${uList?.contains(users?.username) }">class="online"</g:if>
